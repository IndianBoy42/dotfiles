...
local pkgroot = os.getenv "HOME" .. "/.config/xplr/plugins"
package.path = pkgroot .. "/?/src/init.lua"
...
xplr.config.modes.builtin.default.key_bindings.on_key["o"] =
  xplr.config.modes.builtin.selection_ops.key_bindings.on_key["x"]
...
xplr.config.modes.builtin.default.key_bindings.on_key["c"] = xplr.config.modes.builtin.action.key_bindings.on_key["c"]
...
xplr.config.modes.builtin.quit.key_bindings.on_key["l"] = xplr.config.modes.builtin.action.key_bindings.on_key["!"]
...
if true then
  ...
  local plugins = {
    ["comex"] = "sayanarijit",
    ["completion"] = "sayanarijit",
    ["fzf"] = "sayanarijit",
    ["dua-cli"] = "sayanarijit",
    ["zoxide"] = "sayanarijit",
    ["xargs"] = "sayanarijit",
    ["zentable"] = "sayanarijit",
    ["material-landscape2"] = "sayanarijit",
    ["type-to-nav"] = "prncss-xyz",
    ["icons"] = "prncss-xyz",
  }
  for repo, user in pairs(plugins) do
    local dir = pkgroot .. "/" .. repo
    if not isdir(dir) then
      local cmd = "git clone --filter=tree:0 git://github.com/" .. user .. "/" .. repo .. ".xplr.git " .. dir
      os.execute(cmd)
    end
  end
end
